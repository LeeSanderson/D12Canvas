...
There is one counterexample, and it is the gesture whose press does not land on `.diagram-canvas`. ADR 0018 lists the press-time focus transfer among four decisions that "derive from the role alone", and `MinimapPan` is the member with no role — entered directly, never classified. The minimap is host-placed chrome, positioned by the host's own CSS outside `.diagram-container` entirely (ADR 0002, ADR 0015), and ADR 0026 gives it no tab stop and marks it `aria-hidden`, so it cannot hold focus itself. An earlier revision of this section called it canvas-rendered, which was wrong and made the argument weaker than it is: host-placed puts the minimap further from the guard's frame, not nearer.
