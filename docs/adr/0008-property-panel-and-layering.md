...
**Addendum (surfaced while resolving the Custom editor escape hatch ticket):** `Custom`'s RenderFragment is typed `RenderFragment<CustomEditorContext>`, not the bare `RenderFragment<TProps>` described above — `CustomEditorContext` carries the property's current value (already unwrapped from `Props`, the same value every other `EditorKind`'s control works off) plus a `Commit` callback bound to that property. A bare `RenderFragment<TProps>` over an immutable `Props` record would give the author's fragment no way to commit an edit back through the panel's undoable path at all.
