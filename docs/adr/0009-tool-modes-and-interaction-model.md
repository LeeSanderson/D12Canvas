...
| Shift+1 | Zoom to fit all content | ADR 0015 |
| Shift+2 | Zoom to selection | ADR 0015 |
| Shift+0 | Zoom to 100% | ADR 0015 |
...
**Addendum (surfaced while resolving the clipboard and duplication model ticket):** the "Copy/paste, duplicate, select-all shortcuts" rejection above has been resolved rather than reversed — the underlying models it was waiting on are now decided in ADR 0013, which binds `Ctrl+C`, `Ctrl+X`, `Ctrl+V`, `Ctrl+D` and `Ctrl+A`. Three of those are not rows in this table's mechanism at all: copy, cut and paste are handled from the DOM `copy`/`cut`/`paste` events rather than the keydown listener, because the `paste` event is the only permission-free read path. All five additionally require focus within the canvas container, a stricter guard than this table's `isEditableTarget`, since `Ctrl+C` and `Ctrl+A` are contested with any embedding host page. Reconciling that inconsistency across the whole table is the keyboard-parity work's, not this addendum's.

**Addendum (surfaced while resolving the viewport commands and minimap ticket):** this ADR's claim that "right-click on empty canvas opens no menu at all — there is no decided action that would belong there" is **retired**. Its premise was that no decided action belonged there, and that is no longer true: ADR 0011's snap-to-grid toggle and ADR 0015's three viewport commands all do, and the reference bar documents exactly those as a whiteboard's empty-canvas menu. Which of them appear and how the menu is composed is the context-menu decision's, not this addendum's. ADR 0015 also adds the three `Shift`+digit rows to the table above, bound here rather than deferred because a single convention covers all three; they match on `event.code` (`Digit1`/`Digit2`/`Digit0`) so they are layout-independent, and like the clipboard rows they require focus within the canvas container rather than the table's looser `isEditableTarget` guard. Separately, note that `PageUp`/`PageDown` have been bound to zoom in/out since before this table existed and appear in no ADR at all; they belong to the wheel-and-continuous-input decision, which owns the zoom response curve, and ADR 0015 deliberately leaves them alone.
...
**Addendum (surfaced while resolving the pointer gesture arbitration ticket):** ADR 0018 amends the **Escape row** above. "Clear selection, or cancel an in-progress connector drag" named one specific gesture because there was no owner to name instead; it becomes "cancel the active pointer gesture, otherwise clear selection." Nothing else in this table moves — the button and modifier semantics remain the right-button and press-to-drag decision's to amend. This ADR's core claim is *strengthened* rather than touched: a pointer gesture cannot outlive a press, so "no persistent tool modes" now holds by construction rather than by convention. Note also that ADR 0018 deletes every `@onclick` and `@ondblclick` binding on board content in favour of a press count carried on the hit-target classification, so the three double-click meanings this table never documented (enter edit mode, add a custom port, add an edge label) dispatch from the same place as everything else.

**Addendum (surfaced while resolving the right-button and press-to-drag ticket):** ADR 0022 replaces this ADR's **Context menu** paragraph entirely. Right-drag now pans the board, so the menu can no longer open on press: nothing can tell a pan from a menu request until the button comes up. The menu opens on **release**, only when the press stayed inside the 4-pixel drag threshold, and it is anchored at the **press** point. It also resolves the selection at that moment, preserving it for a press inside the current selection, replacing it for a press on an unselected entity, and clearing it for a press on empty canvas. The **empty-canvas menu now exists**, filling in the premise the viewport-commands addendum above had already retired. What appears on either menu is the context-menu decision's, not this ADR's.

The consequence for this table is that the **secondary button is no longer a menu button**. It is a pan button whose sub-threshold release happens to open a menu, which also means a keyboard route to the menu is now missing outright; that belongs to the keyboard-parity work rather than here.

This ADR's core claim is confirmed and *strengthened* rather than weakened. ADR 0022 rejects both a held-space pan quasimode and a Hand tool, so "no persistent tool modes" holds with no exception at all. Ticket 03's proposed restatement into held-versus-entered-versus-one-shot was therefore never needed for pan, though it still stands for the one-shot placement gestures this table already describes.
