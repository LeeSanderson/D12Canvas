...
**Addendum (surfaced while resolving the port affordance ticket):** ADR 0028 amends this ADR in three places, and one of them is a deletion rather than a change.

**The resize-beats-port precedence is retired, not kept as a backstop.** It was recorded above as preserving today's behaviour while expecting the port affordance work to "separate the two geometrically so this rule almost never fires". The `Border partition` separates them by construction — a port span and a resize span are disjoint stretches of the same side, and a corner reserve subtracted before any port is placed keeps corner targets clear of both — so the rule can no longer fire at all. It is replaced by an invariant that can be tested where a dormant precedence cannot: **no two affordance regions on an instance's border intersect, at any zoom, for any port configuration.** That is WCAG 2.2 SC 2.5.8's spacing exception stated as a property, and it is the criterion the concentric geometry failed at every zoom by construction. The rest of the precedence order here is untouched: affordances still beat the `selection-bounds` interior, content still follows paint order, and instances still beat edges.

**`PortHitRadius` becomes `N` = 24 screen pixels**, completing the change ADR 0027 began. One number, defined in C# and published through `ContentStyle`, with the drop floor (`N/2`) and the corner reserve (`N`) derived from it so there is nothing else to keep in step.

**The visual-clamped, hit-unclamped split this ADR deferred is taken at 0.25×.** The drawn dot and handle stop growing below that zoom while their targets keep tracking the full scale, which is tldraw's own rule and which ADR 0011's unbounded zoom-out makes necessary rather than decorative.

Two of this ADR's rules turned out to be load-bearing in ways it did not state. The requirement that **every hit region belong to something visible** is what an invisible partition risks violating, and it survives because a port span's visual is its dot and a resize span's is the instance's own border. And **affordances leaving the render set and the hit set together** is what makes the affordance floor bite before `LodSizeThreshold` does: a default 200×140 instance loses its last port at about 0.34× zoom where LOD swaps at 0.16×, so an `LOD placeholder` is never asked whether it has ports and this ADR's content-versus-affordance split needs no port-specific clause.

**Addendum (surfaced while resolving the keyboard parity ticket):** the keyboard unlock route above becomes library-guaranteed rather than host-dependent, which closes a hole rather than adding a convenience. This ADR names the property panel as the route a keyboard user takes to a locked entity, and the panel is host-placed and optional — ticket 08 found `BoardEditor` mounts none at all. So a keyboard user on a host without a panel had no unlock route, which is precisely the locked-forever failure the participation-versus-reachability rule exists to prevent. ADR 0026 binds `Shift+F10` and the `ContextMenu` key to the menu, so ADR 0023's Unlock row and Unlock All are both reachable with no host cooperation. Locking additionally gains `Ctrl+Shift+L`, on the ground that every other action on a selection in the table has a chord and locking would have been the only one without. Nothing about the lock semantics, the `ChangeLockedCommand`, or the separation rule moves.
