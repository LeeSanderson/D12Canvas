...
**Addendum (surfaced while resolving the state/data model ticket):** registration must validate key uniqueness at composition time — a second `RegisterComponent` call reusing a key already bound to a different `TComponent`/`TProps` pair should throw, not silently overwrite or coexist. The board-level state model (`ComponentInstance.ComponentTypeKey`) assumes the registry guarantees exactly one type per key.

**Addendum (surfaced while resolving the "Canvas renders a Board" ticket):** a registered `TComponent` receives its instance's `Props` value through a single, fixed parameter — `[Parameter] public TProps Props { get; set; }`. `DiagramCanvas`'s registry-resolved rendering binds this exact parameter name when constructing each instance's `DynamicComponent`, so component authors must declare it verbatim to receive their props.
