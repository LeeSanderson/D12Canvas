...
**Amended by ADR 0034:** a colour role paints its value into its own glyph, and three built-in colour props now default to `null` meaning *no author opinion*. A null value has no ink, so a colour role's glyph needs a themed state and a route back to it, the same clear control ADR 0034 adds to the panel. Separately, `PropertyRole` was weighed as the vocabulary a C#-side themed-default resolver would key off and rejected for it: exactly one author-facing role has a token to fall back to, so the mapping layer would serve a single property.
