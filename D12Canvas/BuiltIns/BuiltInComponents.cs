...
// mechanism a host uses for its own types - there is no separate "built-in" path. Called once from
// ServiceCollectionExtensions.AddD12Canvas, after the host's own configure callback has already
// run, so built-ins are appended after any host-registered types.
